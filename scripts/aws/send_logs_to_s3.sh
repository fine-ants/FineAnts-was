...
# 설정값
WORK_DIR="/home/ec2-user/build/production"
LOG_DIR="$WORK_DIR/logs" # 로그 파일이 저장된 디렉토리
BACKUP_DIR="$WORK_DIR/backup" # 압축된 파일을 저장할 임시 디렉토리
S3_BUCKET="s3://fineants2024/production/logs" # S3 버킷 경로
RETENTION_DAYS=7 # 로컬에서 유지할 로그 파일의 보관 기간

# 배포 로그 파일 경로
DEPLOY_FILE="$WORK_DIR/deploy.log"
...
# S3로 업로드
echo "Uploading to S3..." >> $DEPLOY_FILE
